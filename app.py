    ...
    if not api_key:
        ...
        try:
            ...
            with st.spinner('AI가 최신 Imagen 4 모델로 스티커를 그리고 있습니다... (약 10~20초 소요)'):
                # 💡 ImageGenerationModel 클래스 존재 여부 확인 (버전 체크)
                if not hasattr(genai, 'ImageGenerationModel'):
                    st.error("설치된 google-generativeai 라이브러리 버전이 너무 낮습니다.")
                    st.info("💡 **해결 방법:** GitHub의 `requirements.txt` 파일을 열고 `google-generativeai>=0.8.0`으로 수정 후 저장해 주세요.")
                else:
                    # 최신 모델명 설정
            ...
            st.info("💡 힌트: 라이브러리 버전 문제일 가능성이 높습니다. requirements.txt를 확인해 주세요.")
